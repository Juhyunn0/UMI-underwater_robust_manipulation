...
- **무엇:** 독립 하네스 [verify/verify_hydro.py](../verify_hydro.py)가 각 hydro 항을 **외란 OFF**(`disturbance=None`
...
- **방법:** 시뮬레이터는 **무수정**. 알려진 body wrench를 MuJoCo의 독립 외력 버퍼 `data.xfrc_applied`로 주입
  ...
  힘으로 상쇄.
- **실행:** `python verify/verify_hydro.py` (env `robust`). 그림은 [docs/figs/](figs/)에 저장.
...
## 정밀 검증 (`verify/verify_hydro_precise.py`)
...
*재현:* `python verify/verify_hydro_precise.py --tier 1234` (env `robust`; ~25 s; `casadi`·`scipy` 필요).
...
*재현:* `python verify/verify_hydro.py` (env `robust`).
