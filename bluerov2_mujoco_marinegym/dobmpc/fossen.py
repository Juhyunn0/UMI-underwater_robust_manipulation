...
def mass_matrix():
    ...
    ZG_MASS (CG-offset-from-CB inertial coupling) is kept separate from the
    restoring ZG so a plant whose COM sits at the body origin (marinegym) can keep
    the buoyancy restoring while zeroing the m*zg off-diagonals. Falls back to ZG
    when ZG_MASS is absent (original dobmpc params), preserving paper behaviour."""
    m = P.MASS
    zg = getattr(P, "ZG_MASS", P.ZG)
