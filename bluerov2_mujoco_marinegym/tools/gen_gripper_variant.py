...
What gets patched (numbers from compute_payload_inertia.py):
  ...
  4. Three C3 <camera> elements (stereo pair, baseline 7.5 cm + center) at the lens
     ...
     assembly 2026-07-19 — see process_c3_mesh.py; placement numbers come from
     meshes/c3_payload_frames.json, cross-checked against CP.C3_POS).

Run in `robust`:  python gen_gripper_variant.py
"""
import json
import os
import re
...
def build():
    ...
    header = (
        "<!-- GENERATED by gen_gripper_variant.py from bluerov_heavy.xml - DO NOT EDIT "
    ...
    out = _sub_once(
        ...
        f'hydro\'s body-frame drag - see gen_gripper_variant.py). -->',
        "heavy inertial")
    ...
    # C3/mount placement from the Onshape-derived meta (process_c3_mesh.py); the
    ...
    if np.linalg.norm(np.array(meta["c3_centroid_bl"]) - CP.C3_POS) > 1e-3:
        raise RuntimeError("CP.C3_POS disagrees with meshes/c3_payload_frames.json — "
                           "rerun process_c3_mesh.py and update compute_payload_inertia.py")
    g = CP.GRIP_POS - com
    c3 = CP.C3_POS - com
