...
class TrajectoryRecorder:
    ...
    def start(self, now_s: float) -> None:
        ...
        self.frames_dir = self.output_dir / "zed_frames"
        self.frames_dir.mkdir(parents=True, exist_ok=True)
        print(
            "Trajectory recording started. "
            f"ZED frames are being saved in {self.frames_dir}. "
    ...
    def _new_output_dir(self) -> Path:
        ...
        date = timestamp[:8]
        base = self.output_root / date / f"{timestamp}_tagslam_trajectory"
        output_dir = base
        suffix = 1
        while output_dir.exists():
            output_dir = self.output_root / date / f"{timestamp}_tagslam_trajectory_{suffix:02d}"
            suffix += 1
